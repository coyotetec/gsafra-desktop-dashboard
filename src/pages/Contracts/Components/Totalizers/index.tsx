    ...
    <Container>
      ...
      {contracts.length === 0 ? (
        ...
        <div key={contract.id} className="total-contract">
          <span>
            {`${contract.cliente} - ${contract.numeroContrato}`}
          </span>
          <div className="contract-progress">
            <div className="progressbar">
              <div
                className="bar"
                style={{
                  width: contract.porcentagem > 100 ? `${100 - (((contract.porcentagem - 100) * 100) / contract.porcentagem)}%` : `${contract.porcentagem}%`
                }}
              >
              </div>
              ...
              <strong>
                {formatNumber(contract.porcentagem, '%')}
              </strong>
            </div>
            <div className="info">
              <span>
                <strong>Entregue: </strong>
                {formatNumber(contract.totalEntregue, ' Kg')} / {formatNumber(contract.totalEntregue / 60, ' Sacas')}
              </span>
              <span>
                <strong>Saldo: </strong>
                {formatNumber(contract.totalContrato - contract.totalEntregue, ' Kg')} / {formatNumber((contract.totalContrato - contract.totalEntregue) / 60, ' Sacas')}
              </span>
            </div>
          </div>
          <div className="contract-value">
            <strong>{formatNumber(contract.totalContrato, ' Kg')}</strong>
            <strong>{formatNumber(contract.totalContrato / 60, ' Sacas')}</strong>
          </div>
        </div>
      ))}
